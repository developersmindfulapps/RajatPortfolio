...
export default function AdminContactMessagesPage() {
  ...
  const getStatusBadgeClass = (status: string) => {
    switch (status) {
      case "unread": return "bg-red-500/15 text-red-700 dark:text-red-400 border border-red-500/30";
      case "read": return "bg-green-500/15 text-green-700 dark:text-green-400 border border-green-500/30";
      case "archived": return "bg-env-text/10 text-env-muted border border-env-border/30";
      default: return "bg-env-text/10 text-env-muted border border-env-border/30";
    }
  };
          ...
          {messages.map((msg) => {
            ...
            return (
              <Card 
                ...
                hoverEffect={false} 
                className={`
                  bg-env-surface/85 backdrop-blur-md border p-6 flex flex-col transition-all duration-300
                  ${isUnread ? "border-env-text/60 bg-env-surface shadow-md ring-1 ring-env-text/20" : "border-env-border/30 shadow-xs"}
                `}
              >
                ...
                {/* Message Body */}
                <div className="mt-4 bg-env-text/5 p-4 rounded-lg border border-env-border/20">
                ...
                <div className="flex justify-end gap-2.5 border-t border-env-border/20 pt-4 mt-6">
                  ...
                  {isUnread ? (
                    <Button
                      ...
                      className="h-8.5 px-3 text-[10px] font-bold uppercase tracking-wider text-env-text border-env-border hover:bg-env-text/10"
                    >
                      {isMutating === msg._id ? (
